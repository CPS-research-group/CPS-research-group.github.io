...
A major challenge is to come up with a sensible MCS model for automotive with a support to achieve graceful degradation of software elements corresponding to lower critical functionalities under the occurrence of timing fault. A test bed was built in order to benchmark the performance of proposed system model with the existing ones. The testbed is an hardware in the loop platform with TORCS as the vehicle simulator and Texas Instruments Hercules Development Board as the ECU. Applications such as Collision Avoidance, Adaptive Cruise Control, Steering Control etc., are implemented as tasks in FreeRTOS.
******
